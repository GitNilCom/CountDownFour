...
static NSInteger const kCEHeaderHeight = 30;
static NSInteger const kCEFooterHeight = 40;
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    ...
    return kCEHeaderHeight;
}

- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{

    return kCEFooterHeight;
}

- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{

    id model = self.headerDatas[section];
    CETimeCountDownLabel *label=[[CETimeCountDownLabel alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), kCEHeaderHeight)];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    UIView *footerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), kCEFooterHeight)];
    footerView.backgroundColor = [UIColor orangeColor];
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 150, 30)];
    label.center = CGPointMake(label.center.x, kCEFooterHeight/2.0);
    label.text = [NSString stringWithFormat:@"区尾[%ld/%ld]",section,self.footerDatas.count];
    [footerView addSubview:label];
    CETimeCountDownLabel *timeLabel=[[CETimeCountDownLabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(label.frame)+5, 0, CGRectGetWidth(self.view.frame)-CGRectGetWidth(label.frame)-25, kCEHeaderHeight)];
    timeLabel.center = CGPointMake(timeLabel.center.x, kCEFooterHeight/2.0);
...
- (CETimeCountDown *)countDown{
    ...
    //时间格式为时间戳
    countDown.timeStyle = CECountDownTimeStyleTamp;
    return countDown;
}
