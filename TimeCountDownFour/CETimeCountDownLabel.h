...
typedef NS_ENUM(NSInteger ,CETextStlye){

    CETextStlyeNormal = 0,
    CETextStlyeCustom,
    CETextStlyeDDHHMMSSBox,
    CETextStlyeHHMMSSBox,
    CETextStlyeMMSSBox,
    CETextStlyeSSBox,
    CETextStlyeDDHHMMSSChineseBox,
    CETextStlyeHHMMSSChineseBox,
    CETextStlyeMMSSChineseBox,
    CETextStlyeSSChineseBox,
    
};

typedef NS_ENUM(NSUInteger, CETextEffectStlye) {
    CETextEffectStlyeNone                     = 0,
    /**
     ...
     */
    CETextEffectStlyeHollow,

};

//文本对齐方式
typedef NS_ENUM(NSInteger ,CETextAlignmentStlye){
    
    CETextAlignmentStlyeLeftCenter = 0,
    CETextAlignmentStlyeLeftTop,
    CETextAlignmentStlyeLeftBottom,
    CETextAlignmentStlyeCenterTop,
    CETextAlignmentStlyeCenter,
    CETextAlignmentStlyeCenterBottom,
    CETextAlignmentStlyeRightTop,
    CETextAlignmentStlyeCenterRight,
    CETextAlignmentStlyeRightBottom,
    //自定义位置，配合textLeftDeviation和textTopDeviation属性值来使用
    CETextAlignmentStlyeCustom,
    //水平居中，配合textLeftDeviation属性值来使用
    CETextAlignmentStlyeHorizontalCenter,
    //垂直居中 配合textTopDeviation属性值来使用
    CETextAlignmentStlyeVerticalCenter,

};

@interface CETimeCountDownLabel : UILabel

/**
 label显示样式：默认样式：55天05时30分10秒
 如果要自定义样式，需要设置样式为CETextStlyeCustom，并实现CETimeCountDownDelegate自定义文本方法
 */
@property (nonatomic ,assign) CETextStlye textStyle;

/**
 单个文本框的对齐方式
 */
@property (nonatomic ,assign) CETextAlignmentStlye jj_textAlignment;

/**
 文字效果
 */
@property (nonatomic ,assign) CETextEffectStlye effectStlye;
...
/**
 保留过时的值，对CETextStlyeNormal和CETextStlyeCustom有效
 */
...
/**
 最左边单个文本左偏移量 ，对CETextAlignmentStlyeCustom和CETextAlignmentStlyeHorizontalCenter对齐样式有效
 */
@property (nonatomic ,assign) CGFloat textLeftDeviation;

/**
 针对Box风格
 单个文本顶部偏移量，对CETextAlignmentStlyeCustom和CETextAlignmentStlyeVerticalCenter对齐样式有效
 */
...
/**
 描边效果的宽度，文字效果设置为CETextEffectStlyeNoneHollow风格才有效
 */
@property (nonatomic ,assign) CGFloat textHollowWidth;

/**
 描边效果的颜色，文字效果设置为CETextEffectStlyeNoneHollow风格才有效
 */
