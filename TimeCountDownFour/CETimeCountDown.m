...
#import "CETimeCountDownCellManager.h"
#import "CETimeCountDownLabelManager.h"

@interface CETimeCountDown ()<CETimeCountDownCellManagerDelegate ,CETimeCountDownLabelManagerDelegate>

@property (nonatomic ,strong) CETimeCountDownLabelManager *labelManager;
@property (nonatomic ,strong) CETimeCountDownCellManager *cellManager;

@end


@implementation CETimeCountDown
...
- (instancetype)initWithScrollView:(UIScrollView *)scrollView dataList:(NSMutableArray *)dataList{
    ...
    self = [super init];
    if (self) {
        _cellManager = [[CETimeCountDownCellManager alloc] initWithScrollView:scrollView dataList:dataList];
...
#pragma mark ===================CETimeCountDownCellManagerDelegate=================
...
- (NSAttributedString *)cellTimeStringWithModel:(id)model timeLabel:(CETimeCountDownLabel *)timeLabel{
...
- (CETimeCountDownLabelManager *)labelManager{
    
    if (!_labelManager) {
        _labelManager = [[CETimeCountDownLabelManager alloc] init];
...
- (NSAttributedString *)timeStringWithModel:(id)model timeLabel:(CETimeCountDownLabel *)timeLabel{

    //timeKey一定要设置，在初始化视图时设置或者在设置timeLabel其他属性前设置
    if (!timeLabel.timeKey) {
         NSAssert(NO, @"请查看CETimeCountDownLabel类的timeKey属性是否已经设置，如果设置，请检查是否在视图初始化时设置");
    }
    ...
    long long endTimeTamp = [CETimeCountDownDateTool getTimeTampWithStr:serverEndTime timeStyle:self.timeStyle];
...
- (NSAttributedString *)getNowTimeWithStartTimeTamp:(NSInteger )startTimeTamp endTimeTamp:(long long)endTimeTamp timeLabel:(CETimeCountDownLabel *)timeLabel{
...
- (NSAttributedString *)setupLabelTextWithTimeLabel:(CETimeCountDownLabel *)timeLabel{
    ...
    if (timeLabel.days <= 0&&timeLabel.hours<=0&&timeLabel.minutes<=0&&timeLabel.seconds<=0) {
        ...
        timeLabel.minutes = 0;
        timeLabel.seconds = 0;
        if (![CETimeCountDownLabelTextStlyeTool isBoxStyleWithLabel:timeLabel] && timeLabel.days == 0 && timeLabel.hours == 0 && timeLabel.minutes == 0 && timeLabel.seconds ==0 && timeLabel.totalSeconds == 0) {
...
- (NSAttributedString *)setupTextWithLabel:(CETimeCountDownLabel *)timeLabel{
    ...
    if ([self.delegate respondsToSelector:@selector(customTextWithTimeLabel:timeCountDown:)]) {
        ...
        if (![self isNillWithString:customizeText.string] && timeLabel.textStyle == CETextStlyeCustom) {
