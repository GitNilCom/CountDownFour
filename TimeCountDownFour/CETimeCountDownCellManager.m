...
@interface CETimeCountDownCellManager()

@end

@implementation CETimeCountDownCellManager{
...
- (BOOL)setupTabelViewHeaderOrFooterTimeLabelWithView:(UIView *)view{
    ...
    if ([view isKindOfClass:[CETimeCountDownLabel class]]) {
        isTimeCountDownLabel = YES;
        CETimeCountDownLabel *timeLabel = (CETimeCountDownLabel *)view;
        if (timeLabel.model) {
...
- (BOOL)setupTableViewHeaderOrFooterWithSubView:(UIView *)view{
    ...
    for (UIView * contentSubView in view.subviews) {
        if ([contentSubView isKindOfClass:[CETimeCountDownLabel class]]) {
            isTimeCountDownLabel = YES;
            CETimeCountDownLabel *timeLabel = (CETimeCountDownLabel *)contentSubView;
            if (timeLabel.model) {
                [self setupAttributedText:timeLabel model:timeLabel.model];
            }
        }else{
...
- (void)setupTabelViewCellWithTimeLabel:(CETimeCountDownLabel *)timeLabel{
...
- (void)setupAttributedText:(CETimeCountDownLabel *)timeLabel model:(id)model{
...
//是否是自动删除已过时的时间数据
- (BOOL)isAutomaticallyDeletedWithModel:(id)model timeLabel:(CETimeCountDownLabel *)timeLabel{
